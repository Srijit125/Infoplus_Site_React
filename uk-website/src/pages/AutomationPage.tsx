...
import {
  Target, Search, BarChart2, Globe,
  Zap, Server, Activity, RefreshCcw,
  Cpu, Layers, Shield, CheckCircle2,
  ArrowRight, ChevronRight, type LucideIcon,
} from "lucide-react";
import { PageHero } from "../components/shared/PageHero";
import { ScrollReveal } from "../components/ui/ScrollReveal";

type Pillar = { icon: LucideIcon; gradient: string; num: string; title: string; desc: string };
type AutoService = { icon: LucideIcon; gradient: string; title: string; desc: string };
...
const OUTCOMES = [
  {
    ...
    bullets: ["Proactive anomaly detection", "Self-healing remediation", "Zero-downtime operations", "Continuous health validation"],
  },
  {
    icon: Zap,
    ...
    bullets: ["Rapid iteration cycles", "Failure-safe testing environments", "Faster market response", "Resource-efficient pivoting"],
  },
];
...
const AUTO_SERVICES: AutoService[] = [
  { icon: Zap,       gradient: "from-[#381f55] to-[#6128a6]", title: "Automated Predictions", desc: "AI-driven predictive models that anticipate system behaviour, demand spikes, and operational anomalies before they occur." },
  { icon: Server,    gradient: "from-[#1e3a8a] to-[#3b82f6]", title: "IT Automation — Load Balancing & Auto-Scaling", desc: "Intelligent resource orchestration that automatically adjusts infrastructure capacity in real time to match demand." },
  { icon: Activity,  gradient: "from-[#0f766e] to-[#14b8a6]", title: "Solutions to Improve Service Experience", desc: "Automated service desk, incident routing, and resolution workflows that dramatically improve end-user experience." },
  { icon: RefreshCcw, gradient: "from-[#4c1d95] to-[#7c3aed]", title: "Agile Testing Automation", desc: "End-to-end test automation frameworks that integrate into CI/CD pipelines for continuous quality assurance." },
  { icon: Cpu,       gradient: "from-[#78350f] to-[#f59e0b]", title: "Robotic Process Automation (RPA)", desc: "Software bots that replicate repetitive human tasks across applications — freeing your workforce for higher-value work." },
  { icon: Layers,    gradient: "from-[#9a2600] to-[#f85d37]", title: "Industrial Automation", desc: "Automation solutions for manufacturing and industrial environments including SCADA, DCS, and motion control systems." },
  { icon: RefreshCcw, gradient: "from-[#1e3a8a] to-[#3b82f6]", title: "Integrated Automation", desc: "Cross-platform automation that connects disparate systems, APIs, and processes into a seamless, orchestrated workflow." },
];
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <ScrollReveal direction="left" duration={720}>
              ...
              <h2 className="text-[34px] font-bold text-[#111] mt-3 leading-tight mb-5">
                The Next Critical Step<br />
                <span className="text-[#6128a6]">in IT Maturity.</span>
              </h2>
              <p className="text-[15.5px] text-[#555] leading-[1.85] mb-4">
                Automation is the next critical step in IT maturity, building trust into IT systems
                and enabling innovation of new capabilities. Infoplus Technologies delivers automation
                solutions "as-a-service" and has successfully helped various industries with automation
                strategies and implementations.
              </p>
              <p className="text-[15.5px] text-[#555] leading-[1.85] mb-4">
                Our clients not only reduced costs and turnaround times, but were also guided in
                effective redeployment of workforce. Our automation solutions did not kill JOBS.
              </p>
              <p className="text-[15.5px] text-[#555] leading-[1.85]">
                Our carefully selected and nurtured automation team believes and practises that
                automation should provide a higher degree of experimentation and increase agility
              ...
              <div className="bg-[#0d0517] rounded-3xl p-8 relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-[13px] text-white/45 leading-relaxed mb-6">
                    Every automation roadmap starts with 10 key looming questions — from "where to start?"
                    to "what's the winning recipe for success?" We've derived our strategy from these.
                  </p>
                  <div className="flex items-center gap-4 mb-5">
                    <div className="text-[48px] font-black text-white leading-none">10</div>
                    <div>
                      <p className="text-[13px] font-bold text-white">Key Questions</p>
                      <p className="text-[11px] text-white/35">→ 4 strategic pillars</p>
                    </div>
                  </div>
                  <div className="h-px bg-white/10 mb-5" />
                  <div className="grid grid-cols-2 gap-2">
                    {["Detect", "Avoid", "Heal", "Remediate"].map((action, i) => (
                      <div
                        key={i}
                      ...
                      >
                        ...
                        <span className="text-[12px] text-white/60 font-medium">{action}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="container mx-auto px-6 max-w-6xl relative z-10">
          <ScrollReveal direction="fade">
            <div className="text-center mb-12">
              ...
              <h2 className="text-[36px] font-bold text-white mt-2">Two Primary Outcomes</h2>
            </div>
          </ScrollReveal>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            {OUTCOMES.map(({ icon: OIcon, gradient, accentColor, title, desc, bullets }, i) => (
              <ScrollReveal key={i} direction={i === 0 ? "left" : "right"} duration={720} delay={80}>
                <div className="group bg-white/5 border border-white/8 rounded-2xl p-8 hover:border-[#6128a6]/30 hover:bg-white/7 transition-all duration-300 relative overflow-hidden h-full">
                  ...
                  />
                  <div className="relative z-10">
                    <div className={`w-14 h-14 rounded-2xl bg-linear-to-br ${gradient} flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300`}>
                      <OIcon className="w-7 h-7 text-white" />
                    </div>
                    <h3 className="text-[22px] font-bold text-white mb-4 leading-snug">{title}</h3>
                    <p className="text-[14px] text-white/55 leading-relaxed mb-6">{desc}</p>
                    <div className="space-y-2">
                      {bullets.map((b, j) => (
                        <div key={j} className="flex items-center gap-2.5 text-[13px] text-white/60">
        ...
        <div className="container mx-auto px-6 max-w-6xl relative z-10">
          <ScrollReveal direction="fade">
            <div className="text-center mb-14">
              ...
              <h2 className="text-[36px] font-bold text-[#111] mt-2">4 Strategic Pillars</h2>
              <p className="text-[15px] text-[#555] max-w-2xl mx-auto mt-4 leading-relaxed">
                Derived from 10 key industry questions, our automation strategy is built on four
                foundational pillars that guide every engagement.
              </p>
            </div>
          </ScrollReveal>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {PILLARS.map(({ icon: PIcon, gradient, num, title, desc }, i) => (
              <ScrollReveal key={i} direction={i % 2 === 0 ? "left" : "right"} duration={700} delay={i * 90}>
                <div className="group bg-white border border-[#e5e4e7] rounded-2xl overflow-hidden hover:border-[#6128a6]/25 hover:shadow-[0_16px_48px_-8px_rgba(97,40,166,0.10)] hover:-translate-y-1 transition-all duration-300">
                  ...
                  <div className="p-7">
                    <div className="flex items-center gap-4 mb-5">
                      <div className={`w-12 h-12 rounded-xl bg-linear-to-br ${gradient} flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}>
        ...
        <div className="container mx-auto px-6 max-w-6xl">
          <ScrollReveal direction="fade">
            <div className="text-center mb-12">
              ...
              <h2 className="text-[36px] font-bold text-[#111] mt-2">Our Automation Services</h2>
            </div>
          </ScrollReveal>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {AUTO_SERVICES.map(({ icon: SIcon, gradient, title, desc }, i) => (
              <ScrollReveal key={i} direction={i % 2 === 0 ? "left" : "right"} duration={680} delay={60}>
                <div className="group flex items-start gap-4 bg-[#f8f5ff] border border-[#e8e0f7] rounded-xl p-5 hover:bg-white hover:border-[#6128a6]/30 hover:shadow-[0_8px_24px_-4px_rgba(97,40,166,0.10)] transition-all duration-300">
                  <div className={`w-11 h-11 rounded-xl bg-linear-to-br ${gradient} flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform duration-300`}>
            ...
            <div>
              ...
              <div className="space-y-3">
                {AUTO_BENEFITS.map((benefit, i) => (
                  <ScrollReveal key={i} direction="left" delay={i * 70} duration={600}>
                    <div className="flex items-center gap-3 bg-white/5 border border-white/8 rounded-xl px-5 py-3.5 hover:bg-white/8 hover:border-[#6128a6]/30 transition-all duration-200 group">
                      ...
                      <span className="text-[14px] text-white/70 font-medium">{benefit}</span>
                    </div>
                  </ScrollReveal>
                ))}
              </div>
                ...
                <div className="flex flex-wrap gap-2.5">
                  {AUTO_TOOLS.map((tool, i) => (
                    <span
                      ...
                      style={{ animation: `revealFade 400ms ease ${i * 50 + 200}ms both` }}
                    >
                      {tool}
                    </span>
                  ))}
            ...
            <div className="bg-linear-to-br from-[#0d0517] to-[#381f55] rounded-3xl p-14 text-center text-white relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <h2 className="text-[36px] md:text-[42px] font-bold mb-4 leading-tight">
                  Resilient. Intelligent.<br className="hidden md:block" />
                  Automated.
                </h2>
                <p className="text-[16px] text-white/55 max-w-2xl mx-auto mb-10 leading-relaxed">
                  Partner with Infoplus Technologies UK Ltd to build automation solutions that
                  increase agility, reduce costs, and unlock experimentation at scale.
                </p>
