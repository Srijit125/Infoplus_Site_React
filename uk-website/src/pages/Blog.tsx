...
function Blog() {
  const posts = [
    ...
    {
      title:
        "Why Staffing & Consulting Firms Are Shifting to Digital Platforms",
      category: "Consulting",
      date: "Aug 14, 2026",
      image:
            ...
            <Masonry gutter="24px">
              {posts.map((post, i) => (
                ...
                >
                  ...
                  <div
                    className={`absolute inset-0 ${post.image ? "bg-gradient-to-t from-slate-900 via-slate-900/40 to-transparent" : "bg-transparent"} p-8 flex flex-col justify-end`}
                  >
                    <div className="mb-4 flex items-center gap-3">
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${post.image ? "bg-blue-600 text-white" : "bg-white/20 text-white backdrop-blur-sm"}`}
                      >
                        {post.category}
                      </span>
                      <span
                        className={`text-sm ${post.image ? "text-slate-300" : "text-white/80"}`}
                      >
                        {post.date}
                      </span>
                    </div>
                    <h3
                      className={`text-2xl font-bold leading-tight ${post.image || post.bg ? "text-white" : "text-slate-900"} group-hover:-translate-y-2 transition-transform duration-300`}
                    >
                      {post.title}
                    </h3>
                  </div>
